      ...
      >
        {data.map((item, i) => (
          <div key={`${item.username}-${i}`} className="shrink-0">
            <div className="p-10 flex flex-col border items-center gap-y-2.5 rounded-lg bg-muted dark:bg-sidebar min-w-[280px] max-w-[400px] h-auto">
              <Image
                src={item.image}
                alt={item.username}
                width={60}
              ...
              <div className="flex flex-col items-center w-full">
                ...
                <p className="text-center font-mono text-xs mt-2 leading-relaxed w-full break-words whitespace-normal">{item.text}</p>
              </div>
            </div>
          </div>
        ))}
